...
PROVIDER=worker
PROVIDER_STATE_MODULE=src.worker # If no provider state just use the provider itself
...
then
  ...
  pact-verifier --provider-base-url=http://$HOST_NAME:$PORT --pact-url=$PACT_DIR/pacts/$CONSUMER-$PROVIDER.json
else
